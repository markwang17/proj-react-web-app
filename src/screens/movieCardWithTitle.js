...
const MovieCard = ({ movieList, headerString }) => {
    ...
    const renderMovie = (movie) => (
        <Col key={movie.id} md={3} onClick={() => navigate(`/details/${movie.id}`)} style={{ cursor: "pointer" }}>
            {movie.poster_path && <img src={`https://image.tmdb.org/t/p/w300${movie.poster_path}`} alt={movie.title}/>}
            {!movie.poster_path && <img src={process.env.PUBLIC_URL + "/200x300.png"} alt="poster not found"/>}
